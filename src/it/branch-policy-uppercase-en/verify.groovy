...
def versionChanges = buildLog.findAll { it =~ /fr.brouillard.oss.it::branch-policy-uppercase-en::0 -> 1.0.1-MASTER-SNAPSHOT/ }
...
File installedPomFile = new File(localRepositoryPath, "fr/brouillard/oss/it/branch-policy-uppercase-en/1.0.1-MASTER-SNAPSHOT/branch-policy-uppercase-en-1.0.1-MASTER-SNAPSHOT.pom")
log.println( "installedPomFile: " + installedPomFile.isFile() + " " + installedPomFile )
assert installedPomFile.isFile()
def installedPomFileVersion = installedPomFile.readLines().findAll { it =~ /<version>1.0.1-MASTER-SNAPSHOT<\/version>/ }
