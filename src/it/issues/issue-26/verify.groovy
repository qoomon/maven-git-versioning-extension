...
def foundLines = new File("$basedir", "build.log").readLines().findAll { it =~ /fr.brouillard.oss.it.multi.with.parents::main::0 -> 1.0.1-SNAPSHOT/ } 
assert 1 == foundLines.size

// And check that the produced artifact was installed with the good version
File installedPomFile = new File("$basedir" + "/../../../local-repo/fr/brouillard/oss/it/multi/with/parents/main/1.0.1-SNAPSHOT/", "main-1.0.1-SNAPSHOT.pom")
assert installedPomFile.isFile()
assert 1 == installedPomFile.readLines().findAll { it =~ /<version>1.0.1-SNAPSHOT<\/version>/ }.size()

File installedJarPomFile = new File("$basedir" + "/../../../local-repo/fr/brouillard/oss/it/multi/with/parents/child/1.0.1-SNAPSHOT/", "child-1.0.1-SNAPSHOT.pom")
